...
- [ ] **Apply the same 40-minute rule to the existing 303 when merging.** The Kaggle dump is TMDB-derived too, so it almost certainly carries the same shorts (Luxo Jr., Presto, Paperman, Feast, Lava, Piper). Filtering only the new half would leave the catalog inconsistent.
- [ ] Concatenate into `supabase_movies.csv` (**blocked on Kaggle credentials** — the original 303 cannot be regenerated without them, so the merged catalog does not exist yet)
