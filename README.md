...
**Data**: 3 Kaggle sources — "The Movies Dataset" (`movies_metadata.csv` + `keywords.csv`) and MPST (`mpst_full_data.csv`, richer plot synopses matched by exact IMDb ID). Cleaned, then narrowed to a demo scope of Disney + Pixar movies (45,466 raw → 43,270 clean → 303 in scope). All 303 go to Supabase for structured filtering; the 170 with an MPST synopsis (56% coverage) are embedded into Pinecone for semantic search. Wikipedia is fetched live per-candidate rather than pre-indexed. See `scripts/data cleaning rules.md` for the full design rationale.
