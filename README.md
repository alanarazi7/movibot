...
**Chunk 1b (in progress)** closes two gaps found while reviewing that output. The Kaggle catalog is a MovieLens dump ending in **July 2017**, so it cannot answer the project's own demo query for a *"not-too-old Disney movie"* — everything from Encanto to Inside Out 2 is missing. `data_preprocessing/fetch_tmdb_catalog.py` pulls that gap straight from the TMDB API (same id space and column names as the existing rows, so new movies append without reconciliation) and needs only a free TMDB key. Alongside it, `data_preprocessing/title_matching.py` provides a remake-safe `(title, year)` join for the transcript corpora we are evaluating as a richer source for `SceneSearch` — necessary because almost every Disney animated classic has a live-action remake under an identical title, and a silent mis-attachment would make the agent cite a real scene from the wrong film.

## Tests

```bash
pip install -r requirements-dev.txt
pytest tests/ --cov=data_preprocessing --cov-report=term-missing
```
